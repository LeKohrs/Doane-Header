module.exports = function(grunt) {
    ...
    * ---------------------- */
    ...
    grunt.registerTask('drush', 'Send arbitrary commands to Drush on a remove server.', function(method) {
        ...
        if(method == 'cache' && settings.env.platform == 'drupal') {
            var cc_prefix = (settings.env.platform_version == "8") ? 'cr' : 'cc';

            if(options.mode) {
                options.cmd += ' ' + cc_prefix + ' ' + options.mode;
            }
            else {
                options.cmd += ' ' + cc_prefix + ' css-js';
            }

            grunt.log.subhead("Clearing Drupal CSS/JS cache on '" + options.target + "'");
            grunt.log.writeln("Running Drush command: '" + options.cmd + "'");
        }
            ...
            grunt.fail.fatal("There are no arguments or methods for Drush to execute, or this is not a Drupal project.");
        }
        // Execute command
    ...
    grunt.registerTask('deploy_files', 'Wrapper around basic rsync commands', function(method) {
        ...
        options.deleteontarget = grunt.option('delete-on-target');
        ...
        if(method == 'prep') {
            ...
            if(settings.env.platform == 'drupal') {
                ...
                options.excludes = '--exclude==\'.svn\' --exclude=js/ --exclude=css/ --exclude=xmlsitemap/ --exclude=styles/ --exclude=ctools/ --exclude=php/ --exclude=advagg_*';
            ...
            options.flags = '--no-perms --no-owner --no-group';
            ...
            if (options.deleteontarget) {
                options.cmd = 'rsync -aOv ' + options.excludes + ' ' + options.flags + ' --delete-before ' + options.sourcepath + ' ' + options.targetpath;
            }
            else {
                options.cmd = 'rsync -aOv ' + options.excludes + ' ' + options.flags + ' ' + options.sourcepath + ' ' + options.targetpath;
            }
